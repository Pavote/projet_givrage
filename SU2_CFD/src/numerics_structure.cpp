...
void CNumerics::GetInviscidPlessProjFlux(su2double *val_density,
                                    su2double *val_velocity,
                                    su2double *val_enthalpy,
                                    su2double *val_normal,
                                    su2double *val_Proj_Flux) {
  ...
  if (nDim == 2) {
    ...
    val_Proj_Flux[0] = rhou*val_normal[0];
    val_Proj_Flux[1] = (rhou*val_velocity[0])*val_normal[0];
    val_Proj_Flux[2] = rhou*val_velocity[1]*val_normal[0];
    val_Proj_Flux[3] = rhou*(*val_enthalpy)*val_normal[0];

    val_Proj_Flux[0] += rhov*val_normal[1];
    val_Proj_Flux[1] += rhov*val_velocity[0]*val_normal[1];
    val_Proj_Flux[2] += (rhov*val_velocity[1])*val_normal[1];
    val_Proj_Flux[3] += rhov*(*val_enthalpy)*val_normal[1];
    ...
    val_Proj_Flux[1] += rhow*val_velocity[0]*val_normal[2];
    val_Proj_Flux[2] += rhow*val_velocity[1]*val_normal[2];
    val_Proj_Flux[3] += (rhow*val_velocity[2])*val_normal[2];
    val_Proj_Flux[4] += rhow*(*val_enthalpy)*val_normal[2];
...
void CNumerics::GetInviscidPlessProjJac(su2double *val_velocity, su2double *val_energy,
                                   su2double *val_normal, su2double val_scale,
                                   su2double **val_Proj_Jac_Tensor) {
  AD_BEGIN_PASSIVE
  unsigned short iDim, jDim;
...
//  a2 = Gamma-1.0;
  ...
  for (iDim = 0; iDim < nDim; iDim++) {
    ...
    for (jDim = 0; jDim < nDim; jDim++)
      val_Proj_Jac_Tensor[iDim+1][jDim+1] = val_scale*(val_normal[jDim]*val_velocity[iDim]);
    val_Proj_Jac_Tensor[iDim+1][iDim+1] += val_scale*proj_vel;
    //val_Proj_Jac_Tensor[iDim+1][nDim+1] = val_scale*a2*val_normal[iDim];
  }

  val_Proj_Jac_Tensor[nDim+1][0] = val_scale*proj_vel**val_energy;
  for (iDim = 0; iDim < nDim; iDim++)
    val_Proj_Jac_Tensor[nDim+1][iDim+1] = val_scale*(val_normal[iDim]);
  val_Proj_Jac_Tensor[nDim+1][nDim+1] = val_scale*proj_vel;
  AD_END_PASSIVE
}
...
void CNumerics::GetPlessPMatrix(su2double *val_density, su2double *val_velocity,
                           su2double val_enthalpy, su2double *val_normal, su2double **val_p_tensor) {

  su2double vel, rhooc, rhoxc;
  //su2double c2;
  ...
  if (nDim == 2) {
    ...
    vel = val_velocity[0]*val_normal[0]+val_velocity[1]*val_normal[1];

    val_p_tensor[0][0] = -vel;
    val_p_tensor[0][1]=1.0;
    val_p_tensor[0][2]=val_normal[1]/vel;
    val_p_tensor[0][3]=0.0;

    val_p_tensor[1][0]=-val_velocity[0]*vel;
    val_p_tensor[1][1]=0.0;
    val_p_tensor[1][2]=0.0;
    val_p_tensor[1][3]=0.0;

    val_p_tensor[2][0]=-val_velocity[1]*vel;
    val_p_tensor[2][1]=0.0;
    val_p_tensor[2][2]=1.0;
    val_p_tensor[2][3]=0.0;

    val_p_tensor[3][0]=-val_enthalpy*vel;
    ...
    vel = val_velocity[0]*val_normal[0]+val_velocity[1]*val_normal[1]+val_velocity[2]*val_normal[2];

    val_p_tensor[0][0]=-vel;
    val_p_tensor[0][1]= 1.0;
    val_p_tensor[0][2]=val_normal[1]/vel;
    val_p_tensor[0][3]=val_normal[2]/vel;
    val_p_tensor[0][4]=0.0;

    val_p_tensor[1][0]=-val_velocity[0]*vel;
    val_p_tensor[1][1]=0.0;
    val_p_tensor[1][2]=0.0;
    val_p_tensor[1][3]=0.0;
    val_p_tensor[1][4]=0.0;

    val_p_tensor[2][0]=-val_velocity[1]*vel;
    val_p_tensor[2][1]=0.0;
    val_p_tensor[2][2]=1.0;
    val_p_tensor[2][3]=0.0;
    val_p_tensor[2][4]=0.0;

    val_p_tensor[3][0]=-val_velocity[2]*vel;
    val_p_tensor[3][1]=0.0;
    val_p_tensor[3][2]=0.0;
    val_p_tensor[3][3]=1.0;
    val_p_tensor[3][4]=0.0;

    val_p_tensor[4][0]=-val_enthalpy*vel;
    val_p_tensor[4][1]=0.0;
    val_p_tensor[4][2]=0.0;
    val_p_tensor[4][3]=0.0;
...
void CNumerics::GetPlessPMatrix_inv(su2double *val_density, su2double *val_velocity,
                           su2double val_enthalpy, su2double *val_normal, su2double **val_p_tensor) {
  ...
  if (nDim == 2) {
    ...
    val_p_tensor[0][1]=-1.0/(vel*val_velocity[0]);
    val_p_tensor[0][2]=0.0;
    val_p_tensor[0][3]=0.0;

    val_p_tensor[1][0]=1.0;
    val_p_tensor[1][1]=-val_normal[0]/vel;
    val_p_tensor[1][2]=-val_normal[1]/vel;
    val_p_tensor[1][3]=0.0;

    val_p_tensor[2][0]=0.0;
    val_p_tensor[2][1]=-val_velocity[1]/val_velocity[0];
    val_p_tensor[2][2]=1.0;
    val_p_tensor[2][3]=0.0;

    val_p_tensor[3][0]=0.0;
    val_p_tensor[3][1]=-val_enthalpy/val_velocity[0];
    val_p_tensor[3][2]=0.0;
    val_p_tensor[3][3]=1.0;
    ...
    val_p_tensor[1][1]=-val_normal[0]/vel;
    val_p_tensor[1][2]=-val_normal[1]/vel;
    val_p_tensor[1][3]=-val_normal[2]/vel;
    val_p_tensor[1][4]=0.0;
    ...
    val_p_tensor[3][0]=0.0;
    val_p_tensor[3][1]=-val_velocity[2]/val_velocity[0];
    val_p_tensor[3][2]=0.0;
    val_p_tensor[3][3]=1.0;
    val_p_tensor[3][4]=0.0;

    val_p_tensor[4][0]=0.0;
    val_p_tensor[4][1]=-val_enthalpy/val_velocity[0];
    val_p_tensor[4][2]=0.0;
    val_p_tensor[4][3]=0.0;
    val_p_tensor[4][4]=1.0;
