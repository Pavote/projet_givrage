...
void COutput::MergeSolution(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  bool flow           = (( config->GetKind_Solver() == IMPACT            ) ||
                         ( config->GetKind_Solver() == EULER             ) ||
  ...
  switch (Kind_Solver) {
    case IMPACT : case EULER : case NAVIER_STOKES: FirstIndex = FLOW_SOL; if(config->GetWeakly_Coupled_Heat()) SecondIndex = HEAT_SOL; else SecondIndex = NONE; ThirdIndex = NONE; break;
  ...
  if (!config->GetLow_MemoryOutput()) {
    ...
    if ((Kind_Solver == IMPACT) || (Kind_Solver == EULER) || (Kind_Solver == NAVIER_STOKES) || (Kind_Solver == RANS)) {
...
void COutput::SetConvHistory_Header(ofstream *ConvHist_file, CConfig *config, unsigned short val_iZone) {
  ...
  switch (config->GetKind_Solver()) {
    ...
    case IMPACT : case EULER : case NAVIER_STOKES: case RANS :
...
void COutput::SetConvHistory_Body(ofstream *ConvHist_file,
  ...
  if (rank == MASTER_NODE) {
    ...
    bool flow = (config[val_iZone]->GetKind_Solver() == IMPACT) || (config[val_iZone]->GetKind_Solver() == EULER) || (config[val_iZone]->GetKind_Solver() == NAVIER_STOKES) ||
...
void COutput::LoadLocalData_Flow(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  if ((Kind_Solver == NAVIER_STOKES) || (Kind_Solver == RANS) || (Kind_Solver == IMPACT)) {
    Aux_Frict_x = new su2double[geometry->GetnPoint()];
    Aux_Frict_y = new su2double[geometry->GetnPoint()];
    Aux_Frict_z = new su2double[geometry->GetnPoint()];
    Aux_Heat    = new su2double[geometry->GetnPoint()];
    Aux_yPlus   = new su2double[geometry->GetnPoint()];
    Aux_Beta    = new su2double[geometry->GetnPoint()]; // Helo -- nettoyer
    ...
    for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
      if (config->GetMarker_All_Plotting(iMarker) == YES) {
        for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
          ...
          Aux_Heat[iPoint] = solver[FLOW_SOL]->GetHeatFlux(iMarker, iVertex);
          Aux_yPlus[iPoint] = solver[FLOW_SOL]->GetYPlus(iMarker, iVertex);
          Aux_Beta[iPoint] = solver[IMPACT_SOL]->GetCMassOutlet(iMarker, iVertex);
        }
      }
    }
  }
  ...
  for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
    ...
    if (!Local_Halo[iPoint] || Wrt_Halo) {
      ...
      if (!config->GetLow_MemoryOutput()) {
        ...
        if (Kind_Solver == IMPACT) {
          Local_Data[jPoint][iVar] = Aux_Beta[iPoint]; iVar++;
